    ...
    <div className="min-h-screen" style={{ backgroundColor: '#1a2535' }}>
      <div className="mx-auto max-w-[430px] px-4 pb-8">
        <div className="flex items-center gap-3 py-5">
          <BackButton href="/" />
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#f5f0e8' }}>New Lead</h1>
            <p className="text-xs" style={{ color: '#718096' }}>Paste a form submission or describe what the customer said</p>
          </div>
        </div>
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="rounded-xl p-4" style={{ backgroundColor: '#243044', border: '1px solid rgba(184,150,74,0.4)' }}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#b8964a' }}>SMS</span>
                <div className="flex items-center gap-2">
                  <span className="text-xs" style={{ color: '#718096' }}>{result.sms.length} chars</span>
                  <button onClick={() => copyText(result.sms, 'sms')} className="p-1.5 rounded-lg" style={{ backgroundColor: 'rgba(184,150,74,0.1)', color: '#b8964a' }}>
                    {copiedSms ? <Check size={14} /> : <Copy size={14} />}
                  </button>
                </div>
              </div>
              <p className="text-sm leading-relaxed" style={{ color: '#f5f0e8' }}>{result.sms}</p>
            </div>

            {/* Email Card */}
            <div className="rounded-xl p-4" style={{ backgroundColor: '#243044', border: '1px solid rgba(96,165,250,0.4)' }}>
              <div className="flex items-center justify-between mb-2">
              ...
              <p className="text-sm leading-relaxed whitespace-pre-line" style={{ color: '#f5f0e8' }}>{result.email}</p>
            </div>

            <FeedbackWidget tool="lead" outputSummary={result.sms.slice(0, 100)} />

            {!showSaveForm ? (
              <button onClick={() => setShowSaveForm(true)} className="w-full py-3 rounded-xl font-semibold text-sm" style={{ backgroundColor: 'rgba(184,150,74,0.1)', border: '1px solid rgba(184,150,74,0.3)', color: '#b8964a' }}>
                Save as Lead
              </button>
            ) : (
              <div className="rounded-xl p-4 space-y-3" style={{ backgroundColor: '#243044', border: '1px solid rgba(184,150,74,0.3)' }}>
                <h3 className="text-sm font-semibold" style={{ color: '#f5f0e8' }}>Save Lead</h3>
                ...
                <div className="flex gap-2">
                  <button onClick={handleSave} disabled={saving || !saveData.customerName.trim()} className="flex-1 py-2.5 rounded-lg font-semibold text-sm disabled:opacity-50" style={{ backgroundColor: '#b8964a', color: '#1a2535' }}>
                    {saving ? 'Saving...' : 'Save'}
                  </button>
                  <button onClick={() => setShowSaveForm(false)} className="flex-1 py-2.5 rounded-lg text-sm" style={{ backgroundColor: 'rgba(184,150,74,0.1)', border: '1px solid rgba(184,150,74,0.2)', color: '#718096' }}>
                    Cancel
                  </button>
                </div>
              </div>
